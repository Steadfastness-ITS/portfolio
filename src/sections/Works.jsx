...
const Works = () => {
  ...
  return (
    <section id="works" className="py-24 bg-white px-6 md:px-24 min-h-[500px]">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {workList.map((work, index) => (
            <div key={`${work.title}-${index}`} className="group bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-lg hover:shadow-2xl transition-all duration-500 flex flex-col h-full">
              ...
              <div className="relative h-60 w-full overflow-hidden bg-slate-200">
                <img 
                  ...
                  onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/400x300?text=Work+Image'; }}
                />
                ...
                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
                  ...
                  <a 
                    href={work.live === "#" ? undefined : work.live} 
                    target="_blank" 
                    rel="noreferrer" 
                    className={`w-10 h-10 bg-white rounded-full flex items-center justify-center text-slate-900 shadow-md transition-all duration-300 ${work.live === "#" ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#a855f7] hover:text-white'}`}
                    title="Live Demo"
                  >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap gap-2">
                  {work.tags.map((tag, tIdx) => (
                    <span key={`${work.title}-${tag}-${tIdx}`} className="text-[#4f46e5] text-xs font-bold px-1">
                      {tag}
                    </span>
                  ))}
                </div>
